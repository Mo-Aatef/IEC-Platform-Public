...
// --- 2. FETCH MODULE & LESSONS (With Student Context) ---
...
<div class="week-detail-container" style="max-width: 1100px; margin: 0 auto; padding-bottom: 80px;">
    ...
    <div class="lessons-grid-admin">
        ...
        <div class="card lesson-control-card <?php echo $is_unlocked ? 'is-live' : 'is-locked'; ?>">
            ...
            <div class="lc-footer">
                <button class="btn-icon-text" onclick="safeEdit(<?php echo $l['id'] ?? 0; ?>, <?php echo $is_unlocked; ?>)">
                    <i class="fa-solid fa-pen"></i> Edit
                </button>

                <label class="pacing-toggle">
                    <input type="checkbox" 
                           onchange="toggleLesson(this, <?php echo $l['id'] ?? 0; ?>)"
    ...
    <div class="offline-card">
        <form method="POST" id="d7-form">
            ...
            <div class="offline-header">
                <div>
                    <div class="offline-badge">
                        <i class="fa-solid fa-users"></i> Day 7 · Offline Session
                    </div>
                    <div class="guardrail-notice">
                        <i class="fa-solid fa-circle-info"></i> 
                        <strong>Non-Learning Event:</strong> Does not affect student grading or progress.
                    </div>
                </div>
                <div style="text-align: right;">
                    <?php if($d7_msg): ?>
                        <span class="save-feedback success"><i class="fa-solid fa-check"></i> <?php echo $d7_msg; ?></span>
                    <?php elseif($off['updated_at']): ?>
                        <span class="save-feedback normal">Last updated: <?php echo $off['updated_at']; ?></span>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="offline-form-grid">
                <div>
                    <label class="offline-label">Event Title</label>
                    <input type="text" name="title" class="offline-input" value="<?php echo htmlspecialchars($off['title']); ?>">
                </div>
                <div>
                    <label class="offline-label">Date</label>
                    <input type="date" name="session_date" class="offline-input" value="<?php echo $off['session_date']; ?>">
                </div>
            </div>

            <div class="offline-form-grid" style="grid-template-columns: 1fr 1fr 2fr;">
                <div>
                    <label class="offline-label">Start</label>
                    <input type="time" name="start_time" class="offline-input" value="<?php echo $off['start_time']; ?>">
                </div>
                <div>
                    <label class="offline-label">End</label>
                    <input type="time" name="end_time" class="offline-input" value="<?php echo $off['end_time']; ?>">
                </div>
                <div>
                    <label class="offline-label">Location</label>
                    <input type="text" name="location" class="offline-input" value="<?php echo htmlspecialchars($off['location']); ?>">
                </div>
            </div>

            <div class="offline-input-group">
                <label class="offline-label">Notes</label>
                <textarea name="notes" class="offline-input offline-textarea"><?php echo htmlspecialchars($off['notes']); ?></textarea>
            </div>

            <div class="offline-footer">
                <button type="submit" class="btn-save-offline">
                    <i class="fa-solid fa-floppy-disk"></i> Save Logistics
                </button>
            </div>
        </form>
    </div>
...
<script>
    // Helper to update the UI visuals
    ...
    async function toggleLesson(checkbox, id) {
        ...
        try {
            ...
            // 2. GET RAW TEXT RESPONSE (Crucial for debugging)
            ...
            try {
                ...
                // --- THIS IS THE TRAP ---
                // If PHP crashes, it sends HTML/Text, not JSON. We catch it here.
                console.error("Server Response:", rawText);
                alert("SERVER ERROR:\n" + rawText.substring(0, 300) + "...");
                throw new Error("Invalid JSON received");
            }

            // 4. CHECK LOGICAL SUCCESS
            if(!data.success) {
                console.error("Logic Error:", data.message);
                updateVisuals(card, !targetState); // Revert
                alert("Database Error: " + data.message);
            }

        } catch(e) {
            // 5. NETWORK/SYSTEM FAIL
            console.error("System Error:", e);
            updateVisuals(card, !targetState); // Revert
            // Only alert if we haven't already alerted the specific server error
    ...
    function safeEdit(id, isUnlocked) {
        if(!id) { alert("Lesson not created yet."); return; }
        const checkbox = document.querySelector(`input[onchange*="${id}"]`);
        const isLive = checkbox ? checkbox.checked : false;
